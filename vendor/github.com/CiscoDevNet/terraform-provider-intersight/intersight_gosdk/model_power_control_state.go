...
type PowerControlState struct {
	...
	// The status of power rebalancing mode of the chassis. If enabled, this mode allows the system to dynamically reallocate the power between servers depending on their power usage. * `Enabled` - The power group is administratively enabled. * `Disabled` - The power group is administratively disabled.
	PowerRebalancing *string `json:"PowerRebalancing,omitempty"`
	// The status of power save mode of the chassis. If Enabled and the requested power budget is less than available power capacity, the additional PSUs not required to comply with redundancy policy are placed in Power Save mode. * `Enabled` - The power group is administratively enabled. * `Disabled` - The power group is administratively disabled.
