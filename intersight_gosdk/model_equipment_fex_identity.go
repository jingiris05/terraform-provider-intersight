...
// EquipmentFexIdentity The FexIdentity object encapsulates the identity and connectivity context of a Fabric Extender (FEX) in the management system. It unifies vendor, model, serial, and switch-connection details to support consistent inventory, policy deployment, and lifecycle operations for fabric extenders. #### Purpose   A FexIdentity provides the authoritative record of a FEX's identity and its association within a fabric interconnect domain. It enables accurate inventory and health monitoring of fabric extenders, supports targeted configuration and firmware upgrades for specific FEX units, and maintains relationship mapping to the parent network element and device registration. #### Key Concepts   - **Connectivity Context:** Records the switch port and network element reference where the FEX is attached.   - **Unique Identity:** Uses vendor/model/serial plus registration to ensure a one-to-one mapping.   - **Permission Inheritance:** Adopts access controls from the registered device to govern update privileges.   - **Lifecycle Events:** Coordinates FEX discovery, removal, and firmware operations within orchestration workflows.
