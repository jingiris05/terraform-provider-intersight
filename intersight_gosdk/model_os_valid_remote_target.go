...
// OsValidRemoteTarget ValidRemoteTargets provide validated remote boot/install targets for a server based on its server profile, focusing on SAN/iSCSI paths. The results enumerate reachable Fibre Channel LUNs and iSCSI LUNs that can be used as remote install targets. #### Purpose Enable OS installation workflows to discover and present valid remote installation targets (SAN boot style) derived from the server’s configured profile connectivity. #### Key Concepts - **Read-based retrieval**: Exposed via READ to fetch current valid remote target options. - **Remote target types**: Returns `fibreChannelLuns` (with initiator/target WWPNs and LUN ID constraints) and `iscsiLuns` (with initiator vNIC MAC and target IQN validation). - **Profile-derived paths**: Targets are determined from the server profile configuration associated with the server. - **Server association**: `server` identifies the compute.Physical for which remote targets are being reported. - **UI-oriented response shaping**: `src` can indicate orchestration UI usage patterns for response formatting.
