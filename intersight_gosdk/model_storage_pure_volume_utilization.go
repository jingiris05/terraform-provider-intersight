...
// checks if the StoragePureVolumeUtilization type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &StoragePureVolumeUtilization{}

// StoragePureVolumeUtilization The PureVolume object represents a volume entity within a PureStorage FlashArray, serving as a fundamental unit for managing storage capacity and functionality.  #### Purpose   PureVolume is used to organize and control the storage network addresses of client computers, ensuring efficient and secure data storage operations. It plays a critical role in managing storage resources by enabling users to read and interact with data stored on the FlashArray. #### Key Concepts  - **Storage Management:** PureVolume provides a mechanism for organizing storage, allowing for streamlined operations and effective resource allocation.  - **Access Control:** It supports various privilege sets, ensuring that only authorized users can perform actions on the storage array.  - **Integration:** PureVolume is designed to work seamlessly with other objects within the storage network, such as hosts and host groups, facilitating integrated storage solutions.
...
func (o StoragePureVolumeUtilization) ToMap() (map[string]interface{}, error) {
...
func (o *StoragePureVolumeUtilization) UnmarshalJSON(data []byte) (err error) {
	...
	err = json.Unmarshal(data, &varStoragePureVolumeUtilizationWithoutEmbeddedStruct)
	if err == nil {
	...
	err = json.Unmarshal(data, &varStoragePureVolumeUtilization)
	if err == nil {
