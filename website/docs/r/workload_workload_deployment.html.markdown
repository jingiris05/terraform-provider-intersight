...
* `rename_request`:(HashMap) - The request for the renaming operation on the workload deployment object. 
This complex property has following sub-properties:
  + `action`:(string) The action to be taken for the rename operation on an object.* `None` - No action is to be taken for the rename request.* `Rename` - The object is to be renamed with the new name in the request. 
  + `new_name`:(string) The new name for the object. This name will be used to rename all objects associated with it. 
  + `object_type`:(string) The fully-qualified name of the instantiated, concrete type.The value should be the same as the 'ClassId' property. 
  + `status`:(string)(ReadOnly) The status of the rename operation for an object.* `None` - No rename operation is in progress, the last rename operation succeeded, or rename has not been performed on the object.* `Updating` - The object is currently being renamed.* `UpdateScheduled` - The rename request for the object has been accepted and will be processed. 
