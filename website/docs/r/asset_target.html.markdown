...
* `claimed_time`:(string)(ReadOnly) The date at which the target was claimed to this account. For pre-provisioned targets this is the time at which the claim was completed. 
* `connections`:(Array)
This complex property has following sub-properties:
...
* `pre_claim`:(bool) A pre-claim of an embedded target. Does not require a security token to be provided at target create, target will be created in a pre-claim state. 
