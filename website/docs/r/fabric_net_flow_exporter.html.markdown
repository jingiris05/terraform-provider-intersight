...
* `source_interface`:(HashMap) - Netflow exporter source interface, used as the source address for netflow export packet. 
This complex property has following sub-properties:
  + `object_type`:(string) The fully-qualified name of the instantiated, concrete type.The value should be the same as the 'ClassId' property. 
  + `switch_aip`:(string) Switch IP for the export interface. 
  + `switch_asubnetmask`:(string) Subnet mask for the export interface network. 
  + `switch_bip`:(string) Switch IP for the export interface. 
  + `switch_bsubnetmask`:(string) Subnet mask for the export interface network. 
  + `vlan_id`:(int) VLAN ID for the export interface. 
* `tags`:(Array)
This complex property has following sub-properties:
...
* `nr_version`:(int)(ReadOnly) Version of flow record format exported in exporter packet.  The value of this field is 9 for the current version. 
...
```
$ terraform import intersight_fabric_net_flow_exporter.example 1234567890987654321abcde
``` 
