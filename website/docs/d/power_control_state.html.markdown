...
* `power_rebalancing`:(string) The status of power rebalancing mode of the chassis. If enabled, this mode allows the system to dynamically reallocate the power between servers depending on their power usage.* `Enabled` - The power group is administratively enabled.* `Disabled` - The power group is administratively disabled. 
