...
subcategory: "iam"
layout: "intersight"
page_title: "Intersight: intersight_iam_ldap_provider_inventory"
description: |-
        ### Overview
...
# Data Source: intersight_iam_ldap_provider_inventory
### Overview
The LDAP Provider object represents a single LDAP server that an endpoint will use for authentication. It is a component of an LdapPolicy.
#### Purpose
The purpose is to define the connection details for one LDAP server. An LdapPolicy can contain a list of LdapProvider objects, creating an ordered list of servers that the endpoint will attempt to contact for authentication.
...
To access the ith object of the results obtained, use `data.intersight_iam_ldap_provider_inventory.<custom_name>.results[i].<propertyname>`.
...
* `server`:(string) LDAP Server Address, such as an IP address or hostname. 
