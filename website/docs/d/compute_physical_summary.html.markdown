...
* `front_panel_lock_state`:(string) The actual front panel state of the server.* `Unlock` - Front Panel of the server is set to Unlocked state.* `Lock` - Front Panel of the server is set to Locked state. 
